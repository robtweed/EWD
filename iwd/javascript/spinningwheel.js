...
SpinningWheel = {
	...
	create: function () {
		...
		document.getElementById(iWD.currentPage).appendChild(div);
		...
		// Create HTML slot elements
              var liStyle = " style='padding:0px 10px 0px 10px;color:black;border-top:0px;'" ;
		for (l = 0; l < this.slotData.length; l += 1) {
			...
			for (i in this.slotData[l].values) {
                        var theId = '' ;
                        if (l == 1) theId=" id='day" + i + "'" ;
				out += '<li' + theId + liStyle + '>' + this.slotData[l].values[i] + '<' + '/li>';
			}
			ul.innerHTML = out;
	...
	destroy: function () {
		...
		//document.body.removeChild(document.getElementById('sw-wrapper'));
              document.getElementById(iWD.currentPage).removeChild(document.getElementById('sw-wrapper'));
	},
	...
	getSelectedMonth: function () {
   		...
   		var index = -Math.round(this.slotEl[0].slotYPosition / this.cellHeight);
            var month = this.slotData[0].values[index+1] ;
            return month ;
	},

	getSelectedYear: function () {
		var year;
   		var index = -Math.round(this.slotEl[2].slotYPosition / this.cellHeight);
            var count = 0 ;
		for (l in this.slotData[2].values) {
		   if (count == index) {
			year = this.slotData[2].values[l];
			break;
		   }	
		   count += 1;
            }
			...
			for (l in this.slotData[i].values) {
				if (count == index) {
					...
					values.push(this.slotData[i].values[l]);
                              //alert("i=" + i + "; index=" + index + "value=" + this.slotData[i].values[l]);
					break;
				}
	...
	scrollStart: function (e) {
		...
		for (var i = 0; i < this.slotEl.length; i += 1) {
            ...
            if (iWD.SpinningWheelUse == 'calendar') {
               var mon30 = {
                  'Apr': '',
	     	      'Jun': '',
		      'Sep': '',
		      'Nov': '',
               ...
               if (this.getSelectedMonth() == 'Feb') {
                   document.getElementById('day' + 30).style.display = 'none' ;
                   document.getElementById('day' + 31).style.display = 'none' ;
                   if (this.getSelectedYear() % 4 == 0) {
                      document.getElementById('day' + 29).style.display = '' ;
                      this.slotEl[1].slotMaxScroll = -1232 ;
                   }
                   else {
                      document.getElementById('day' + 29).style.display = 'none' ;
                      this.slotEl[1].slotMaxScroll = -1188 ;
                   }
               }
               else if (this.getSelectedMonth() in mon30) {
                   document.getElementById('day' + 29).style.display = '' ;
                   document.getElementById('day' + 30).style.display = '' ;
                   document.getElementById('day' + 31).style.display = 'none' ;
                   this.slotEl[1].slotMaxScroll = -1276 ;
               }
               else {
                   document.getElementById('day' + 29).style.display = '' ;
                   document.getElementById('day' + 30).style.display = '' ;
                   document.getElementById('day' + 31).style.display = '' ;
                   this.slotEl[1].slotMaxScroll = -1320 ;
               }
            }
